...
test: dist
	mkdir dist/out/ ;
	conda run -n base python make/env.py test_jsonhold --python=3.11 --recreate >/dev/null;
	conda run -n test_jsonhold pip install dist/*.tar.gz >/dev/null;
	conda run -n test_jsonhold python run_tests.py > dist/out/tests_out.txt 2> dist/out/tests_err.txt || true;
	conda run -n test_jsonhold pip install mypy >/dev/null;
	conda run -n test_jsonhold python -m mypy --exclude build --exclude dist --strict . > dist/out/mypy_dir_out.txt 2> dist/out/mypy_dir_err.txt || true;
	conda run -n test_jsonhold python -m mypy --strict -p datahold > dist/out/mypy_pkg_out.txt 2> dist/out/mypy_pkg_err.txt || true;
